...
以降では, 中央のルータで正常に経路交換ができているか確認するため, r1の挙動を見ます.
...
送信側(TX)も受信側(RX)も正常にパケットの送受信ができていることがわかります.
```
...
## まとめ
今回はIS-ISを触ってみました.
同じリンクステート型のIGPであるOSPFと比べるとあまり聞き馴染みのないプロトコルですが, 
